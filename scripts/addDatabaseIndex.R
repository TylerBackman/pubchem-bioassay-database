...
if(is.na(commandArgs(trailingOnly=TRUE)[1])){
    outputDatabase <- "working/indexedBioassayDatabase.sqlite"
}
...
dbGetQuery(con, "CREATE INDEX IF NOT EXISTS targets_aid ON targets (aid)")

# old indicies (rethink)
...
# dbGetQuery(con, "CREATE INDEX IF NOT EXISTS targetIndex ON domains (target)")
